...
if __name__ == "__main__":
    # Sample text
    sample_text = """One of the other reviewers has mentioned that after watching just 1 Oz episode 
    you'll be hooked. <br />They are right, as this is exactly what happened with me."""
    ...
    # Stemming
    tokens_stemmed = preprocessor.apply_stemming(tokens_no_stop)
    print(f"After stemming ({len(tokens_stemmed)} tokens):")
    print(tokens_stemmed[:20])
    print("\n" + "="*50 + "\n")
    
    # All in one
    result = preprocessor.preprocess(sample_text)
    print("Complete preprocessing in one call:")
    print(result[:20])
